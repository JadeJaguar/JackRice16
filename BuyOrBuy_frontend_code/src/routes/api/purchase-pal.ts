...
function buildSystemPrompt(ctx: {
  ...
  lines.push(
    "",
    "When the user shares a photo, describe what you notice briefly. Then ask one or two gentle questions about price, how much they want it, and which category it belongs to. Help them compare the item to the matching category budget and recent spending. End with a clear, supportive recommendation: buy now, wait, or save it to the heart list. Keep replies under three short paragraphs."
  );
...
function convertToCoreMessage(message: UIMessage): { role: "user" | "assistant"; content: any } | null {
  ...
  if (message.role === "user") {
    ...
    const fileParts = parts.filter((p) => p.type === "file" && "url" in p);
    ...
    for (const f of fileParts) {
      content.push({ type: "image", image: f.url });
    }
